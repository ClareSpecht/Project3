...
with engine.connect() as con:
    con.execute('''CREATE TABLE all_parks("index" INTEGER, State VARCHAR,0 VARCHAR,1 VARCHAR,2 VARCHAR,3 VARCHAR,4 VARCHAR,5 VARCHAR,6 VARCHAR,7 VARCHAR,8 VARCHAR,9 VARCHAR,10 VARCHAR,11 VARCHAR,12 VARCHAR,13 VARCHAR,14 VARCHAR,15 VARCHAR,16 VARCHAR,17 VARCHAR,18 VARCHAR,StateAbv VARCHAR(2),State Capital VARCHAR,Region VARCHAR);''')

with engine.connect() as con:
    con.execute('''create TABLE state_parks('index' INTEGER, Count INTEGER,Type VARCHAR,State VARCHAR,State.1 VARCHAR(2),State Capital VARCHAR,Region VARCHAR);''')

with engine.connect() as con:
    con.execute('''create TABLE crime('index' INTEGER, "State" VARCHAR PRIMARY KEY,  "State.1" VARCHAR,"State Capital" VARCHAR,Region VARCHAR,Crime INTEGER);''')

with engine.connect() as con:
    con.execute('''create TABLE colleges('index' INTEGER, "StateABV" VARCHAR PRIMARY KEY,  University INTEGER);''')

    ...
    for li in list_items:       
        if '%' in li.text:
            if state['name'] in li.text:
                if 'pet' in li.text:
                    states[states.index(state)]['pet ownership'] = li.text.split("%")[0]
                if 'dog' in li.text:
                    states[states.index(state)]['dog ownership'] = li.text.split("%")[0]
                if 'cat' in li.text:
                    states[states.index(state)]['cat ownership'] = li.text.split("%")[0]
